...
def main(path_to_simconfig, newidx,chunk_size, path_to_positions_folder):
    ...
    ids, data, population = getSimulationInfo(path_to_BlueConfig, newidx)
    ...
    for idx, i in enumerate(ids): # Iterates through node_ids and gets segment positions
        ...
        m, somaPos = getMorphology(population,i)
        somaPos = somaPos[:,np.newaxis]
...
if __name__=='__main__':
    ...
    path_tosimconfig = sys.argv[1] #simulation_condif with one-timestep simulation outputting a compartment report
    ...
    main(path_tosimconfig, newidx, chunk_size, path_to_positions_folder)
