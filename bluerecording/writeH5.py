...
def writeH5File(path_to_simconfig,segment_position_folder,outputfile,numFilesPerFolder,sigma=0.277,path_to_fields=None):
    ...
    #data = pd.DataFrame(data, columns=pd.MultiIndex.from_tuples(tuple(map(tuple,data_frame.ids)), names=['id','section']), index=data_frame.times) # Writes compartment report as pandas dataframe
