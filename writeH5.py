...
def getElectrodePositions(outputfile):
    ...
    for electrode in file['electrodes'].keys():
        
        if electrode != 'electrode_grid':

            positions.append(file['electrodes'][electrode]['position'][:]) # Take electrode positions from h5 coefficient file
...
if __name__=='__main__':
    ...
    if len(sys.argv)>6: # Specify either conductance or a potential field, not both
